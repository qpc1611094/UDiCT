...
import random
import dataload_segc19
from torch.utils.data import DataLoader
import matplotlib.pyplot as plt
import numpy as np
import SimpleITK as sitk
import tools
from tools import binary_poss
import argparse
import other_models
import dense_unet
...
argparser.add_argument('--soft', type=str, help='soft', default='mse')
...
argparser.add_argument('--lr', type=float, help='lr', default=3e-4)
argparser.add_argument('--datas', type=float, help='datas', default=0.1)
argparser.add_argument('--EPOCH', type=int, help='EPOCH_num', default=400)
...
argparser.add_argument('--tail_name', type=str, help='tail_name', default="")
...
savename="SegC19_lacrm_"+args.model_name+"_"+str(args.datas)+"_"+str(args.begin_epoch)+"_"+args.rank+"_"+str(args.mix_beta)+"_"+"batch"+str(args.batchsize)+"_"+str(args.EPOCH)+"_"
...
savename = savename+"_"+args.tail_name
...
trainset=dataload_segc19.funcset(mode="train",train_num=args.datas)
train_loader=DataLoader(trainset, batch_size=batchsize, shuffle=True, num_workers=8,drop_last=True)
valset=dataload_segc19.funcset(mode="val",train_num=args.datas)
val_loader=DataLoader(valset, batch_size=1, shuffle=False, drop_last=False)
testset=dataload_segc19.funcset(mode="test",train_num=args.datas)
...
def train(epoch=0):
    ...
    for i, train_datas in enumerate(train_loader):
        img_l, lung_l, target_l, img_u, lung_u, name = train_datas
        ...
        if (args.LACRM and epoch>=args.begin_epoch):
            ...
            if args.rank=='ul':
                ...
                unc_rank = torch.randperm(2*batchsize).cuda()
...
def val(epoch=0):
    global flag, val_dice_best
    dice = 0.0
    num = 0.0
    model.eval()
    for i, test_datas in enumerate(val_loader):
        img_l, lung_l, target_l, img_u, lung_u, name = test_datas
        ...
        num += 1
        del img_l, img_u, target_l

    return dice/num


def test(epoch, save_path="", save=False):
    global flag, test_dice_best
    dice = 0.0
    num = 0.0
    loss_accu=0.0
    model.eval()
