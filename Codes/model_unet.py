...
#vgg16/resnet/
# fcn/unet/deeplabv1v2v3v3+
#rcnn/ / /yolov1v2v3v4/polar mask

#cam/vat/seltraining/teacher-stuent
...
class Unet(nn.Module):
    ...
    def __init__(self,
        ...
        for i, [filters_in, filters_out] in enumerate(zip(decoder_filters_in, decoder_filters)):
            ...
            #                                               use_bn=decoder_use_batchnorm,dropout=True))
            # else:
            #     self.upsample_blocks.append(UpsampleBlock(filters_in, filters_out,
            #                                               skip_in=shortcut_chs[num_blocks-i-1],
            #                                               parametric=parametric_upsampling,
        ...
        self.noise_gen = Uniform(-1, 1)
        # self.conv4cons = nn.Conv2d(decoder_filters[-1],self.args.category_nums,3,1,1)
    ...
    def forward(self, input_1, noise = False):
        ...
        for skip_name, upsample_block in zip(self.shortcut_features[::-1], self.upsample_blocks):
            ...
            # print (skip_name)
            # if noise and skip_name!="5":
            #     noise_sf = self.noise_gen.sample(skip_features.shape[1:]).to(skip_features.device).unsqueeze(0)
            #     skip_features = skip_features.mul(noise_sf) + skip_features
