...
def get_list(mode,num):
    if mode=="ann":
        ...
        f = open(os.path.join(data_root,'data_sample'+str(num)+'.pkl'), 'rb')
        l_list = pickle.load(f)["ann"]
        return [os.path.join(img_root, item) for item in l_list]
    elif mode=="unann":
        img_root = os.path.join(data_root,'train/images')
        f = open(os.path.join(data_root,'data_sample'+str(num)+'.pkl'), 'rb')
        u_list = pickle.load(f)["unann"]
        return [os.path.join(img_root, item) for item in u_list]
    elif mode=="val":
...
def move(img,lung,target,rate=0.3):
    ...
    rate = (random.random()*0.2-0.1)+rate
    
    px = int(img.shape[0]*rate)
    if flag==0:
...
class funcset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if (self.mode=="ann") or (self.mode=="unann"):
            ...
            img_l, target_l = load_data(img_path, mode=self.mode)
            img_l = self.transform(img_l)
