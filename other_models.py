...
class AttU_Net(nn.Module):
    ...
    def __init__(self, args, img_ch=3, output_ch=1,decoder_filters=(256, 128, 64, 32, 16),
                 parametric_upsampling=True,decoder_use_batchnorm=True):
        super(AttU_Net, self).__init__()

        filters = [64, 128, 256, 512, 512]
        self.args = args
        self.backbone_name = args.backbone_name
        self.backbone, self.shortcut_features, self.bb_out_name = mu.get_backbone(self.backbone_name,
    ...
    def forward(self, x, noise=False):
        ...
        e6, features = self.forward_backbone(x)
        val_list=[]
        for key in features.keys():
            val_list.append(features[key])
        e1=val_list[0]
        e2=val_list[1]
        e3=val_list[2]
        e4=val_list[3]
        e5=val_list[4]
        if noise:
            noise_5 = self.noise_gen.sample(e5.shape[1:]).to(e5.device).unsqueeze(0)
            e5 = e5.mul(noise_5) + e5
            # noise_4 = self.noise_gen.sample(e4.shape[1:]).to(e4.device).unsqueeze(0)
            # e4 = e4.mul(noise_4) + e4
            # noise_3 = self.noise_gen.sample(e3.shape[1:]).to(e3.device).unsqueeze(0)
            # e3 = e3.mul(noise_3) + e3
            # noise_2 = self.noise_gen.sample(e2.shape[1:]).to(e2.device).unsqueeze(0)
            # e2 = e2.mul(noise_2) + e2
...
class NestedUNet(nn.Module):
    ...
    def __init__(self, args,in_ch=3, out_ch=1):
    ...
    def forward(self, x, noise = False):
        ...
        x4_0=val_list[4]
        if noise:
            noise_4 = self.noise_gen.sample(x4_0.shape[1:]).to(x4_0.device).unsqueeze(0)
            x4_0 = x4_0.mul(noise_4) + x4_0
            noise_3 = self.noise_gen.sample(x3_0.shape[1:]).to(x3_0.device).unsqueeze(0)
            x3_0 = x3_0.mul(noise_3) + x3_0
            noise_2 = self.noise_gen.sample(x2_0.shape[1:]).to(x2_0.device).unsqueeze(0)
            x2_0 = x2_0.mul(noise_2) + x2_0
            noise_1 = self.noise_gen.sample(x1_0.shape[1:]).to(x1_0.device).unsqueeze(0)
            x1_0 = x1_0.mul(noise_1) + x1_0
