...
class Inf_Net(nn.Module):
    ...
    def forward(self, x, noise = False):
        ...
        if noise:
            noise_2 = self.noise_gen.sample(x2.shape[1:]).to(x2.device).unsqueeze(0)
            noise_3 = self.noise_gen.sample(x3.shape[1:]).to(x3.device).unsqueeze(0)
            noise_4 = self.noise_gen.sample(x4.shape[1:]).to(x4.device).unsqueeze(0)
            x2 = x2.mul(noise_2) + x2
            x3 = x3.mul(noise_3) + x3
            x4 = x4.mul(noise_4) + x4
